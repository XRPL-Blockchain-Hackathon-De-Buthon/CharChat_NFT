...
async function main() {
  ...
  const AIChatbotNFT = await hre.ethers.getContractFactory("AIChatbotNFT");
  const aiChatbotNFT = await AIChatbotNFT.deploy(
    "AI Chatbot NFT",                      // 이름
    "ACNFT",                              // 심볼
    "You are a friendly AI assistant.",   // 초기 프롬프트
    hre.ethers.parseEther("0.1")         // 초기 가격 (0.1 ETH)
  );
  ...
  const ChatbotToken = await hre.ethers.getContractFactory("ChatbotToken");
